...
##  ======================================================================
##
##  Copyright (c) 2001 David Ljung Madison, All rights reserved.
##
##  Original software and this documentation:
...
The B<eperl> program is the I<Embedded Perl 5 Language> interpreter.  
It is designed for general ASCII file generation with the philosophy 
of I<embedding> the Perl 5 program code into the ASCII data instead 
of the usual way where you embed the ASCII data into a Perl 5 program 
(usually by quoting the data and using them via C<print> statements). 
...
I<entirely> evaluated by only one instance of the Perl 5 interpreter.
...
strings or not.  B<All instances of end delimiters will end the embedded perl>.
In other words, if you want to use an end delimiter in your perl blocks,
you either need to escape them (such as ``C<\E<lt>\:>'') or change your
delimiters.
...
The Parse::ePerl(3) package method that comes with the C version of ePerl
...
ePerl does not currently support I<SetUID> switching since I haven't had
time to make sure that this can be done safely. If you know enough about
I<SetUID> and can implement this, I'd love to see a patch.  If you need
I<SetUID> switching, see the original ePerl.
...
I<path> all ePerl begin and end delimiters are removed, #include is converted
...
The answers are: First, you can put really I<any> Perl code into the 
ePerl blocks which are valid Perl. Second, ePerl does I<not> provide 
...
objects. So, browse to the Comprehensive Perl Archive Network (CPAN) via
...
=item B<-d> I<name>=I<value>, B<--define>=I<name>=I<value>
...
=item B<-D> I<name>=I<value>, B<--setenv>=I<name>=I<value>
...
=item B<-B> I<begin_delimiter>, B<--block-begin>=I<begin_delimiter>
...
=item B<-E> I<end_delimiter>, B<--block-end>=I<end_delimiter>
...
=item B<-o> I<outputfile>, B<--outputfile>=I<outputfile>
...
STDOUT before executing it. Only for debugging problems with the inputfile
conversion.

=item B<-X>, B<--heavy_debug>

This sets heavy debug mode.  Only the internally created Perl script is
output, it is not executed.

=item B<-I> I<directory>, B<--includedir>=I<directory>
...
=item B<-V>, B<--ingredients>

Same as option B<-v> (supported for ePerl compatibility)
...
This allows you to read STDIN from your eperl script, which CGIs that 
use POST will need to do - though this is less safe since someone 
might be able to alter your script before you run it.  You can change 
the $TMPDIR in eperl to a non globally writable directory to protect 
against this.
...
(where --eval is the default behavior).  Requires I<IO::String> module.
